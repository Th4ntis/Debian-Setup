...
install() {
    ...
    sudo apt install -y linux-headers-$(uname -r) adb acpi bleachbit build-essential cifs-utils cups curl dialog dkms fastboot flameshot flatpak fonts-powerline fswatch gimp git gnome-software-plugin-flatpak gparted htop idle3 libreoffice lm-sensors make net-tools nload nmap openvpn openssh-server pssh python3 python3-pip python3-setuptools python3-venv screen steam terminator thunderbird tmux ttf-mscorefonts-installer upower vim wireshark xsel zsh
    echo -e "\n $greenplus Complete! \n"
    check_de
    timeshift_install
    brave_install
